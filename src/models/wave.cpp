...
SI::model::Wave::Wave(const size_t waveNumber) : waveNumber(waveNumber), fadeInTimer(60), fadeOutTimer(300), opacity(0)
{
        ...
        try {
                ...
                throw std::runtime_error("Failed to load 'wave" + std::to_string(waveNumber) +
                                         ".json': " + exception.what());
        }
}
...
void SI::model::Wave::update()
{
        ...
        enemies.erase(std::remove_if(enemies.begin(), enemies.end(),
                                     [](const std::weak_ptr<Enemy>& enemy) { return !enemy.lock(); }),
                      enemies.end());
...
const std::string& SI::model::Wave::getTitle() const { return title; }

size_t SI::model::Wave::getWaveNumber() const { return waveNumber; }
...
void SI::model::Wave::parseWave()
{
        ...
        if (!rFile.is_open()) {
                if (waveNumber == 0)
                        throw std::runtime_error("at least wave0.json must be present in data/waves");
                deleteThis();
                return;
        }
...
void SI::model::Wave::positionRow(const std::vector<std::shared_ptr<Enemy>>& row, float& y)
{
        ...
        if (remainingWidth < 1)
                throw std::runtime_error("Not enough space for all enemies a row ");
...
uint8_t SI::model::Wave::getOpacity() const { return opacity; }