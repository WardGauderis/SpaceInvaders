...
SI::model::Shield::Shield(const utils::Vector& position)
    : PhysicalEntity(0, {4 * ShieldSegment::size().x, 3 * ShieldSegment::size().y}, position, {0, 0}, true)
{
        ...
        for (size_t x = 0; x < segments.size(); ++x) {
                for (size_t y = 0; y < segments[x].size(); ++y) {
                        ...
                        if (x == 1 && y == 0)
                                type = ShieldSegment::Type::leftTop;
                        else if (x == 2 && y == 0)
                                type = ShieldSegment::Type::rightTop;
                        else if (x == 0 && y == 2)
                                type = ShieldSegment::Type::rightBottom;
                        else if (x == 3 && y == 2)
                                type = ShieldSegment::Type::leftBottom;
                        else
                                type = ShieldSegment::Type::full;
...
bool SI::model::Shield::collidesWithSegment(const int x0, const int y0, const std::shared_ptr<PhysicalEntity>& entity)
{
                                             ...
                                             const std::shared_ptr<PhysicalEntity>& entity)
{
        ...
        if (auto enemy = std::dynamic_pointer_cast<Enemy>(entity))
                radius += std::min(enemy->getValue() / 2 + 5, 30);
        ...
        for (int x = -radius; x < radius; ++x) {
                for (int y = -radius; y < radius; ++y) {
                        ...
                        if (chance(generator()) == 0)
                                continue;
                        if (!correct(X0, Y0, X1, Y1))
                                continue;

                        segments.at(static_cast<size_t>(X0))
                            .at(static_cast<size_t>(Y0))
                            .points.at(static_cast<size_t>(X1))
                            .at(static_cast<size_t>(Y1)) = false;
                }
        }
...
utils::Vector SI::model::Shield::convert(const int x0, const int y0, const int x1, const int y1) const
{
        return position - size / 2 +
               ShieldSegment::size() * utils::Vector(static_cast<float>(x0), static_cast<float>(y0)) +
