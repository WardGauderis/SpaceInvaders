...
utils::Assets::Assets()
		: music(std::make_shared<sf::Music>()), bold(std::make_shared<sf::Font>()),
		  normal(std::make_shared<sf::Font>()), spy(std::make_shared<sf::Texture>()) {
...
const std::shared_ptr<sf::Texture>& utils::Assets::getSpy() const {
	return spy;
}
