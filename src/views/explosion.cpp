...
{
        sprite.setSize(
            utils::Transformation::get().convertDistance<float>(model::Explosion::ExplosionParticle::size()));
        notify();
}
...
void SI::view::Explosion::notify()
{
        ...
        auto opacity = static_cast<uint8_t>(
            255 * std::pow(static_cast<float>(timer.getTime()) / static_cast<float>(timer.getCooldown()), 3));
        if (m->getTeam())
                sprite.setFillColor(sf::Color(59, 200, 200, opacity));
        else
                sprite.setFillColor(sf::Color(200, 106, 59, opacity));
}
...
int SI::view::Explosion::drawOrder() { return -2; }
